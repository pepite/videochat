...
class JSCam {
	...
	public static function main():Void {
		...
		if (null != camera) {
			...
			display = _root.attachMovie("clip", "video", _root.getNextHighestDepth());
...
*/
	...
	private static function _stream():Void {
		...
		// byte[][] : byte[0] [2] line 0 - 2nd pixel
		var picture:Array = new Array();
		var row = "%";
		for (var i = 0; i < 240; ++i) {
			var byte:Array = new Array();
			for (var j=0; j < 320; ++j) {
				row+= buffer.getPixel32(j, i);
				row+= ";";
				//byte.push(buffer.getPixel32(j, i));
				//
			}
			//picture.push(byte)	;
			row+= "+";
			//ExternalInterface.call('webcam.debug', "error", byte);
			//ExternalInterface.call("webcam.onSave", byte);
		}
